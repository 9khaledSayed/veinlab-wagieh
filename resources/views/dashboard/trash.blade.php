...
@endpush
@section('title') {{ __("Trash") }} @endsection
...
<div class="row justify-content-center">
    ...
    <div class="trashTab col-md-4 col-12 card  text-center h2 py-5 mb-0 text-hover-primary" data-model="FundingOrganization" style="cursor: pointer">
        <i class="fas fa-coins headerIcons"></i>
        <span>{{ __("Funding Organizations") }}</span>
    </div>
...
@push('scripts')
    ...
    <script>
        ...
        tableHeaderColumns.set('FundingOrganization', `<th>#</th>
        ...
        dataTableColumns.set('FundingOrganization', [
                {data: 'id'},
                {data: 'name_ar'},
                {data: 'name_en'},
                {data: 'image'},
        ...
        dataTableColumnsDefs.set('Car', [
                ...
                },
                {
                    ...
                    render: function (data, type, row) {
                        ...
                        return `<a class="d-block overlay" style="height:47px;" data-fslightbox="lightbox-basic" href="${getImagePath('Cars', data)}">
                                    <!--begin::Image-->
                                    <div class="overlay-wrapper bgi-no-repeat bgi-position-center bgi-size-cover card-rounded"
                                         style="height:56px;width:100px;border-radius:4px;margin:auto;background-image:url('${getImagePath('Cars', data)}');background-size:contain;">
                                    </div>
                                    <!--end::Image-->
        ...
        dataTableColumnsDefs.set('FundingOrganization', [
                {
                    targets: 4,
                    render: function (data, type, row) {
                        if ( data == 1)
                ...
                },
                {
                    targets: 3,
                    render: function (data, type, row) {
                        return `<img style="height:50px;width:50px;border-radius:4px" src="${getImagePath('FundingOrganizations', data)}" class="me-3" >`;
                    }
                },
                {
                    targets: -1,
                    ...
                    render: function (data, type, row) {
                        ...
                        return `
                            ...
                            ${ canRestore ? `<!--begin::Menu item-->
                                    <div class="menu-item px-3">
                                        <a href="#" class="menu-link px-3 d-flex justify-content-between restore-row" data-row-id="${row.id}" data-type="${translate('funding organization')}" >
                                        <span> ${translate('Restore')} </span>
                                        <span>  <i class="fas fa-share text-primary"></i> </span>
                                        </a>
