        ...
        datatable = $("#kt_datatable").DataTable({
            ...
            ajax: {
                data: function () {
                    ...
                    if(FROMDATE || TODATE){
                        datatable.DataTable().ajax.url(`/dashboard/waiting-labs/pending?page=${info.page + 1}&per_page=${info.length}&patient=${PAITiENT_ID}&status=${STATUS}&from_date=${FROMDATE}&to_date=${TODATE}`);
                    }else{
                        datatable.DataTable().ajax.url(`/dashboard/waiting-labs/pending?page=${info.page + 1}&per_page=${info.length}&patient=${PAITiENT_ID}&status=${STATUS}`);
                    }
                }
            },
            columns: [
            ...
            columnDefs: [
                {
                    ...
                    render: function (data, type, row) {
                        ...
                        return `<a href="/dashboard/waiting-labs/pending?status=1&patient=${row.patient.id}" style="color:#5b0efa;"> ${row.patient.name_ar} </a>`;
