        ...
        datatable = $("#kt_datatable").DataTable({
            ...
            ajax: {
                data: function () {
                    ...
                    if(FROMDATE || TODATE){
                        datatable.DataTable().ajax.url(`/dashboard/waiting-labs/finished?page=${info.page + 1}&per_page=${info.length}&patient=${PAITiENT_ID}&status=${STATUS}&from_date=${FROMDATE}&to_date=${TODATE}`);

                    }else{
                        datatable.DataTable().ajax.url(`/dashboard/waiting-labs/finished?page=${info.page + 1}&per_page=${info.length}&patient=${PAITiENT_ID}&status=${STATUS}`);
                    }
            ...
            columnDefs: [
                {
                    ...
                    render: function (data, type, row) {
                        ...
                        return `<a href="/dashboard/waiting-labs/finished?status=2&patient=${row.patient.id}" style="color:#5b0efa;"> ${row.patient.name_ar} </a>`;
