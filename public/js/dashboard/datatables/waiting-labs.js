        ...
        datatable = $("#kt_datatable").DataTable({
            ...
            ajax: {
                data: function () {
                    ...
                    if(FROMDATE || TODATE){
                        datatable.DataTable().ajax.url(`/dashboard/waiting-labs?page=${info.page + 1}&per_page=${info.length}&patient=${PAITiENT_ID}&from_date=${FROMDATE}&to_date=${TODATE}`);
                    }else{
                        datatable.DataTable().ajax.url(`/dashboard/waiting-labs?page=${info.page + 1}&per_page=${info.length}&patient=${PAITiENT_ID}`);
                    }
                }
            },
            columns: [
                {data: 'id'},
                {data:null,name:'patients.name'},
                {data:null},
                {data:null},
                {data:null},
                {data: 'status'},
                ...
                },
                {
                    ...
                    render: function (data, type, row) {
                        if(data === 1){
                            ...
                            return `<span class="badge badge-light-danger">${translate('Pending')}</span>`;
                        }else{
                            return `<span class="badge badge-light-success">${translate('Finished')}</span>`;
                        }
                     },
                },
